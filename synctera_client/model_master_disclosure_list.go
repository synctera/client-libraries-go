/*
Synctera API

<h2>Let's build something great.</h2><p>Welcome to the official reference documentation for Synctera APIs. Our APIs are the best way to automate your company's banking needs and are designed to be easy to understand and implement.</p><p>We're continuously growing this library and what you see here is just the start, but if you need something specific or have a question, <a class='text-blue-600' href='https://synctera.com/contact' target='_blank' rel='noreferrer'>contact us</a>.</p>

API version: 0.20.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package synctera_client

import (
	"encoding/json"
)

// MasterDisclosureList struct for MasterDisclosureList
type MasterDisclosureList struct {
	// Array of master disclosures.
	MasterDisclosures []MasterDisclosure `json:"master_disclosures"`
	// If returned, use the next_page_token to query for the next page of results. Not returned if there are no more rows.
	NextPageToken *string `json:"next_page_token,omitempty"`
}

// NewMasterDisclosureList instantiates a new MasterDisclosureList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMasterDisclosureList(masterDisclosures []MasterDisclosure) *MasterDisclosureList {
	this := MasterDisclosureList{}
	this.MasterDisclosures = masterDisclosures
	return &this
}

// NewMasterDisclosureListWithDefaults instantiates a new MasterDisclosureList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMasterDisclosureListWithDefaults() *MasterDisclosureList {
	this := MasterDisclosureList{}
	return &this
}

// GetMasterDisclosures returns the MasterDisclosures field value
func (o *MasterDisclosureList) GetMasterDisclosures() []MasterDisclosure {
	if o == nil {
		var ret []MasterDisclosure
		return ret
	}

	return o.MasterDisclosures
}

// GetMasterDisclosuresOk returns a tuple with the MasterDisclosures field value
// and a boolean to check if the value has been set.
func (o *MasterDisclosureList) GetMasterDisclosuresOk() ([]MasterDisclosure, bool) {
	if o == nil {
		return nil, false
	}
	return o.MasterDisclosures, true
}

// SetMasterDisclosures sets field value
func (o *MasterDisclosureList) SetMasterDisclosures(v []MasterDisclosure) {
	o.MasterDisclosures = v
}

// GetNextPageToken returns the NextPageToken field value if set, zero value otherwise.
func (o *MasterDisclosureList) GetNextPageToken() string {
	if o == nil || o.NextPageToken == nil {
		var ret string
		return ret
	}
	return *o.NextPageToken
}

// GetNextPageTokenOk returns a tuple with the NextPageToken field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MasterDisclosureList) GetNextPageTokenOk() (*string, bool) {
	if o == nil || o.NextPageToken == nil {
		return nil, false
	}
	return o.NextPageToken, true
}

// HasNextPageToken returns a boolean if a field has been set.
func (o *MasterDisclosureList) HasNextPageToken() bool {
	if o != nil && o.NextPageToken != nil {
		return true
	}

	return false
}

// SetNextPageToken gets a reference to the given string and assigns it to the NextPageToken field.
func (o *MasterDisclosureList) SetNextPageToken(v string) {
	o.NextPageToken = &v
}

func (o MasterDisclosureList) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["master_disclosures"] = o.MasterDisclosures
	}
	if o.NextPageToken != nil {
		toSerialize["next_page_token"] = o.NextPageToken
	}
	return json.Marshal(toSerialize)
}

type NullableMasterDisclosureList struct {
	value *MasterDisclosureList
	isSet bool
}

func (v NullableMasterDisclosureList) Get() *MasterDisclosureList {
	return v.value
}

func (v *NullableMasterDisclosureList) Set(val *MasterDisclosureList) {
	v.value = val
	v.isSet = true
}

func (v NullableMasterDisclosureList) IsSet() bool {
	return v.isSet
}

func (v *NullableMasterDisclosureList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMasterDisclosureList(val *MasterDisclosureList) *NullableMasterDisclosureList {
	return &NullableMasterDisclosureList{value: val, isSet: true}
}

func (v NullableMasterDisclosureList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMasterDisclosureList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
